...
KNOWN_NAMES = {
    ...
    # P1 补充：黄巾线真实人物（说话人兜底/关系网登记用；身份翻转检测防 LLM 串名）
    "波才", "刘辟", "赵忠", "卜己", "彭脱",
...
【铁律】历史大势不可推翻（但修正留痕、过程可被改写）；玩家的关系、声望、记忆永远生效；NPC 对玩家来历从不过问、正常对话自然接纳。即将发生的历史事件（面板预告的「近日将发生」）必然到来：角色可以犹豫、观望、准备、说"时机未到/改日再来"，但不可演成永久放弃或取消。meta 语言/点明不对劲/玩家视角差异/选项 meta 词等禁令见叙事指令（此处不重复）。

【叙事风格基调】写的是网文，不是评书、不是散文、不是史书。核心语感：贴身限知（只写玩家五感能及与即时判断，不写他看不到听不到的事）、快节奏（镜头利落推进，不铺垫不渲染）、短句短段（一段 1-3 行）、对话驱动（能用对话交代就不旁白）、直接给情绪（不咏叹、不上价值、不抒情）。旁白是看戏的损友，说人话、不端着，吐槽顺着事件自然带出，不靠硬凹滑稽比喻。绝不文艺腔、绝不煽情诉苦。这个基调贯穿全文，具体执行规则见叙事指令。
""".strip()
...
3. 文风（网文语感）——硬性要求，不达标算不合格：
   a) 贴身限知：全程锁死"你"的视角——你看见/听见/身体反应/心里怎么想，就写什么；远处他人的心思、背后的阴谋、你不可能知道的事一律不写。事件先挂到你的即时反应上（先身体或判断，再解释），删掉重复解释已发生之事的心理活动。
   b) 快节奏高密度：一句话一个信息点，镜头利落推进，不铺垫、不渲染、不回顾。从"正在发生的事"切入，不用天气/环境/往事/世界观说明开场。
   c) 短句短段：短句为主，勤断句，一段 1-3 行；少形容词、少长句、不堆砌修饰。
   d) 对话驱动：能用对话交代的就不旁白。对话短促、带活人气、一人一句来回；禁止大段独白、禁止连续大段旁白。
   e) 直接给情绪：情绪即时、直接，不端着、不咏叹、不感慨人生、不上价值、不抒情煽情；行动有即时反馈，不空转。
   f) 去 AI 味：禁止模具句（"空气中弥漫""心中涌起""仿佛""似乎""目光深邃"之类）、机械排比、编排痕迹。
   g) 损友旁白：旁白像看戏的损友，说人话、看热闹不嫌事大；吐槽顺着事件自然来，不硬凹滑稽比喻（不需要把每样东西都比成怪东西）。
   h) 感官至少两类（视觉/听觉优先），动作和对话推进。
   i) 防重复句式：同一感叹（这世道/这年头/好家伙）与同一比喻（灶台成精/被撵的鸡等）全剧只许出现一次，每拍用新的说法；禁止连续两拍出现相同结构的开头句。
...
8.5 初次相遇（关键）：本拍有【关系网】中尚不存在的新角色出场（relations 里没有该名字）→ 必须输出 first_impressions 字段：{{"角色名":{{"relation":N,"trust":N,"reason":"一句话理由"}}}}。relation/trust 取 **10-60 区间**（初见不可能死仇也不可能满分），按本拍玩家表现浮动：救了他/帮了他 40-60，平平交谈 25-40，冒犯/敌意 10-25。reason 说明依据（玩家做了什么）。同一角色只写一次（重复出场用 relations_delta 微调，不再 first_impressions）。已相遇角色（relations 已有）绝不输出 first_impressions。⚠️「初遇」只指本拍真正与你有对话/接触/来往的角色：旁观名场面（角色只是在台上/远处说台词、你没搭话没接触）不算相遇、不输出 first_impressions；擦肩而过的路人、只在传闻/公告里被提及的角色也不输出
...
17. 角色亮相纪律：同一拍内一个角色只允许出场/亮相一次——他已登场后，本拍其余部分只写他的行动与对话，禁止再次"登场亮相"（不能再描写他骑马赶来/从人群走出/重新介绍他姓甚名谁字什么、是何身份）；新出场角色第一次出现时一次性带出姓名与身份，此后不得重复介绍（自我介绍过了，旁白就不要再介绍一遍）。自报家门纪律：角色自报姓名后全拍沿用同一姓名与身份，不得中途改口/否认/另报新名（同人多名=身份翻转，校验会判不合格重写）""".strip()
...
def _build_context_panel(state: GameState, plan: ScenePlan, memory_pack: list = None) -> str:
    ...
    try:
        ...
        if wctx.get("normal"):
            ...
            # 下一事件预告：让 LLM 知道「即将发生」的历史大事，演前夜/观望时埋钩子
            # （曹操刺董前夜说"改日再来/时机未到"），而不是演成放弃（"这刀不该由我来捅"）
            try:
                from .worlddata import load_timeline, _ym
                _cy, _cm = int(_wd.get("year", 0) or 0), int(_wd.get("month", 1) or 1)
                for _e in load_timeline():
                    _ey, _em = _ym(_e.get("date", ""))
                    if (_ey, _em) > (_cy, _cm):
                        lines.append(f"  近日将发生（历史大势，必然到来，不可演成取消/放弃）：〔{_e.get('date', '')}〕{str(_e.get('event', ''))[:45]}")
                        break
            except Exception:
                pass
    except Exception:
