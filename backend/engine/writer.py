...
2. 玩家视角差异经玩家内心/观察自然呈现（如：你记得史书上写的是'黄巾'……），但世界侧一切正常
...
6. 时空跳跃（跨年/大段路程）须显式交代（如'数月后''几天路程'），不得无标记硬切
7. 世界差异克制（黄金/黄巾等）：无论首次还是后续都只是背景细节，不展开、不吐槽、不反复念叨，靠"被动遇见"自然带出（见旗号/听口号顺势一句）；派系名称'黄金军''黄金兵'每场至多 1-2 次，其余用代称（贼军/溃兵/那支人马/叛军/他们）；口号只按锁定台词逐字出现
...
10. 玩家动作离谱/越权/meta（改世界规则/召唤现代/作弊上帝/命令不可能）：**世界不得真的改变**——乐子人幽默拒绝（如"你咋不上天呢？"），动作滑稽落空、无实际后果；选项须含"换个说法/再想想"重输出口（可作额外第 4 个选项，不挤占 2-3 个常规选项）；NPC 把他的话当疯话自然接住
11. 活世界感：世界在自我转动——①行路/等待写真实时光流逝（赶路写路程、休息写日夜更替），世界随日期推进有回应；②自然带出底色（阶段大势/本地点生态/近期事件，取自面板 🌏/🌐），如逃难流民、新告示、远处火光；③NPC 有自己的路要赶、要避的祸；④克制：底色不是主菜，不抢主线、不打断节奏。若面板有 🗞 与你有关的天下事（strong，如曹操差人送信/仇家追到客栈）应主动汇入本拍、玩家可现场应对（躲/应/追/装不知），选择成为"活的选择点"（§3.4）
...
15. 选项 text/effect 严禁 meta 词与现代词出口给 NPC（如"穿越者""现代""剧本"）；玩家向 NPC 说出异常认知时，NPC 以世界逻辑自然接住或当他疯话""".strip()
